...
#define CLIENT_ID                       "6a436acccbb0cf6bb96a80f9_E53_IA_0_0_2026063009"
#define USERNAME                        "6a436acccbb0cf6bb96a80f9_E53_IA"
#define PASSWORD                        "9557e637792724b9bf91624c67f6864678f26a272a616f5a12a8ff78e03649ca"
...
static void ia_lcd_show_sensor(ia_report_t *report)
{
    ...
    lcd_show_string(0, 100, (uint8_t *)line, LCD_BLACK, LCD_WHITE, 16, 0);
}
...
void ia_deal_cmd_msg(cmd_t *cmd)
{
    ...
    else if (0 == strcmp(cJSON_GetStringValue(obj_cmdname), "电机控制"))
    {
        ...
        if (0 == strcmp(cJSON_GetStringValue(obj_para), "ON"))
        {
            ...
            motor_set_status(ON);
            printf("Motor On\n");
            ia_lcd_show_status("Air ON", "Motor ON");
        }
        else
        {
            ...
            printf("Motor Off\n");
            ia_lcd_show_status("Air OFF", "Motor OFF");
        }
        cmdret = 0;
    }
...
void iot_cloud_ia_thread()
{
    ...
    FlashInit(); 
    printf("[IA] boot iot_cloud_ia_thread\n");
    printf("[IA] wifi ssid=%s password=%s\n", ROUTE_SSID, ROUTE_PASSWORD);
    printf("[IA] client_id=%s\n", CLIENT_ID);
    printf("[IA] username=%s\n", USERNAME);
    printf("[IA] mqtt password=%s\n", PASSWORD);
    ...
    printf("[IA] SetWifiModeOn begin\n");
    SetWifiModeOn();
    printf("[IA] SetWifiModeOn returned, begin MQTT init\n");
...
void iot_cloud_ia_example()
{
    ...
    if (lcd_init() == 0)
    {
        ...
        lcd_fill(0, 0, LCD_W, LCD_H, LCD_WHITE);
        lcd_show_string(0, 20, (uint8_t *)"Smart Home", LCD_BLUE, LCD_WHITE, 24, 0);
        lcd_show_string(0, 70, (uint8_t *)"Cloud Ready", LCD_RED, LCD_WHITE, 24, 0);
    }
    else
    {
        m_lcd_ready = 0;
        printf("[LCD] init failed, display disabled.\n");
    }
