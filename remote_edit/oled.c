...
static OledI2cCandidate m_oled_i2c_candidates[] =
{
    {
        1,
        {
        ...
        "I2C1_M1 GPIO0_PC1/PC2"
    },
    {
        1,
        {
            .scl =  {.gpio = GPIO0_PA3, .func = MUX_FUNC3, .type = PULL_NONE, .drv = DRIVE_KEEP, .dir = LZGPIO_DIR_KEEP, .val = LZGPIO_LEVEL_KEEP},
            .sda =  {.gpio = GPIO0_PA2, .func = MUX_FUNC3, .type = PULL_NONE, .drv = DRIVE_KEEP, .dir = LZGPIO_DIR_KEEP, .val = LZGPIO_LEVEL_KEEP},
            .id = FUNC_ID_I2C1,
            .mode = FUNC_MODE_M2,
        },
        "I2C1_M2 GPIO0_PA2/PA3"
    },
    {
        1,
        {
            .scl =  {.gpio = GPIO0_PB7, .func = MUX_FUNC4, .type = PULL_NONE, .drv = DRIVE_KEEP, .dir = LZGPIO_DIR_KEEP, .val = LZGPIO_LEVEL_KEEP},
            .sda =  {.gpio = GPIO0_PB6, .func = MUX_FUNC4, .type = PULL_NONE, .drv = DRIVE_KEEP, .dir = LZGPIO_DIR_KEEP, .val = LZGPIO_LEVEL_KEEP},
            .id = FUNC_ID_I2C1,
            .mode = FUNC_MODE_M0,
        },
        "I2C1_M0 GPIO0_PB6/PB7"
    },
    {
        2,
        {
...
static unsigned int m_oled_i2c_bus = OLED_I2C_BUS;
static unsigned short m_oled_i2c_address = OLED_I2C_ADDRESS;
#endif
static unsigned int m_oled_i2c_failed = 0;
...
static inline void write_iic_command(unsigned char iic_command)
{
    ...
    ret = LzI2cWrite(m_oled_i2c_bus, m_oled_i2c_address, buffer, 2);
    if (ret != 0)
    {
...
#if OLED_I2C_ENABLE
static unsigned int oled_probe_i2c(void)
{
    unsigned int candidate_count = sizeof(m_oled_i2c_candidates) / sizeof(m_oled_i2c_candidates[0]);
    unsigned short addresses[] = {0x3C, 0x3D};
    unsigned char buffer[2] = {0x00, 0xAE};
    unsigned int i;
    unsigned int j;

    for (i = 0; i < candidate_count; i++)
    {
        m_i2cBus = m_oled_i2c_candidates[i].io;
        m_oled_i2c_bus = m_oled_i2c_candidates[i].bus;
        if (I2cIoInit(m_i2cBus) != LZ_HARDWARE_SUCCESS)
        {
            printf("[OLED] probe %s I2cIoInit failed\n", m_oled_i2c_candidates[i].name);
            continue;
        }
        if (LzI2cInit(m_oled_i2c_bus, m_i2c_freq) != LZ_HARDWARE_SUCCESS)
        {
            printf("[OLED] probe %s LzI2cInit failed\n", m_oled_i2c_candidates[i].name);
            continue;
        }
        for (j = 0; j < sizeof(addresses) / sizeof(addresses[0]); j++)
        {
            if (LzI2cWrite(m_oled_i2c_bus, addresses[j], buffer, 2) == LZ_HARDWARE_SUCCESS)
            {
                m_oled_i2c_address = addresses[j];
                printf("[OLED] found at %s bus=%u addr=0x%02x\n", m_oled_i2c_candidates[i].name, m_oled_i2c_bus, m_oled_i2c_address);
                return 0;
            }
        }
        printf("[OLED] no response at %s\n", m_oled_i2c_candidates[i].name);
        LzI2cDeinit(m_oled_i2c_bus);
    }
    return 1;
}
#endif
...
unsigned int oled_init()
{
    ...
    if (oled_probe_i2c() != 0)
    {
        printf("[OLED] probe failed: no OLED response on known I2C pins\n");
        return 1;
    }
#endif
