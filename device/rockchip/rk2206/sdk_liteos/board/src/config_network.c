...
void set_default_wifi_config(void)
{
    ...
    snprintf(config.sn, sizeof(config.sn), "LZ01");
    snprintf(config.product, sizeof(config.product), "小凌派");
    snprintf(config.factory, sizeof(config.factory), "凌睿智捷");
    snprintf(config.mode, sizeof(config.mode), "STA");
    snprintf(config.ssid, sizeof(config.ssid), AP_SSID);
    snprintf(config.password, sizeof(config.password), AP_PASSWORD);
    snprintf(config.route_ssid, sizeof(config.route_ssid), ROUTE_SSID);
    snprintf(config.route_password, sizeof(config.route_password), ROUTE_PASSWORD);
    config.hwaddr[0] = 0x10;
    config.hwaddr[1] = 0xdc;
    config.hwaddr[2] = 0xb6;
    config.hwaddr[3] = 0x91;
    config.hwaddr[4] = 0x00;
    config.hwaddr[5] = 0x00;
    config.ip[0] = 192;
    config.ip[1] = 168;
    config.ip[2] = 2;
    config.ip[3] = 10;
...
void set_wifi_config(wifi_config_t wifi_config)
{
    ...
    VendorSet(VENDOR_ID_SN,                wifi_config.sn,             sizeof(wifi_config.sn));
    VendorSet(VENDOR_ID_PRODUCT,           wifi_config.product,        sizeof(wifi_config.product));
    VendorSet(VENDOR_ID_FACTORY,           wifi_config.factory,        sizeof(wifi_config.factory));
...
void get_wifi_config(printf_fn pfn, wifi_config_t *wifi_config)
{
    ...
    pfn("%s:           %s\n", WIFI_CONFIG_KEY_SN, wifi_config->sn);
    pfn("%s:      %s\n", WIFI_CONFIG_KEY_PRODUCT, wifi_config->product);
    pfn("%s:      %s\n", WIFI_CONFIG_KEY_FACTORY, wifi_config->factory);
    pfn("%s:         %s\n", WIFI_CONFIG_KEY_MODE, wifi_config->mode);
    pfn("%s:      %s\n", WIFI_CONFIG_KEY_AP_SSID, wifi_config->ssid);
    pfn("%s:    %s\n", WIFI_CONFIG_KEY_AP_PASSWD, wifi_config->password);
    pfn("%s:   %s\n", WIFI_CONFIG_KEY_ROUTE_SSID, wifi_config->route_ssid);
    pfn("%s: %s\n", WIFI_CONFIG_KEY_ROUTE_PASSWD, wifi_config->route_password);
    LOS_Msleep(10);
    
    pfn("%s:          %02x:%02x:%02x:%02x:%02x:%02x\n",
        WIFI_CONFIG_KEY_MAC,
